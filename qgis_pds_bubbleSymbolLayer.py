...
class PdsBubbleSymbolLayer(QgsSimpleMarkerSymbolLayer):

    LAYERTYPE="BubbleSymbolLayer"
    ...
    def __init__(self, props):
        ...
        self.labelSize = 7.0
        self.diagrammStr = props
        self.templateStr = None
        # self.labelDataSums = None

        QgsMessageLog.logMessage('SET PROPERTY ERROR: ' + str(props), 'BubbleSymbolLayer')

        self.attrsUsed = {OLD_NEW_FIELDNAMES[1], PdsBubbleSymbolLayer.LABEL_OFFSETX, PdsBubbleSymbolLayer.LABEL_OFFSETY,
                          PdsBubbleSymbolLayer.BUBBLE_SIZE, PdsBubbleSymbolLayer.DIAGRAMM_LABELS, PdsBubbleSymbolLayer.DIAGRAMM_LABELS}
        ...
        try:
            ...
            self.diagrammStr   = props["diagrammStr"]      if "diagrammStr" in props else u'';
            self.templateStr   = props["templateStr"]      if "templateStr" in props else u'';
        ...
        try:
            if len(self.diagrammStr) > 1:
                ...
                for d in self.diagrammStr:
                    d1 = MyStruct(**d.__dict__)
                    self.diagrammProps.append(d1)
    ...
    def layerType(self):
        return PdsBubbleSymbolLayer.LAYERTYPE
    ...
    def clone(self):
        m = PdsBubbleSymbolLayer(self.properties())
